...
public class AuthFilter implements Filter{
	...
	|| uri.endsWith(PAYMENT) || uri.endsWith(ADD_USER) || uri.endsWith(DELETE_USER) || uri.endsWith(EDIT_USER)||uri.endsWith(MANAGE_USER) || uri.endsWith(ADD_USER)|| uri.endsWith(SYSTEM_REPORTS)||uri.endsWith(HOME) || uri.endsWith(MANAGE_USER) || uri.endsWith(ROOT))  {
	chain.doFilter(request, response);
	...
	|| uri.endsWith(CONTACT) || uri.endsWith(RESERVATION) || uri.endsWith(PAYMENT) ||
	 uri.endsWith(BOOKING_HISTORY) ||uri.endsWith(PROFILE)) {
	chain.doFilter(request, response);
